...
class MomentumHybrid(Momentum):
	...
	def initialize(self, model,
		...
		trial_function      = TrialFunction(model.HV)
		...
		# SIA
		self.resid   = (R_x + R_y)*dx
		self.mom_Jac = derivative(self.resid, U, trial_function)
	...
	def form_obj_ftn(self, integral, kind='log', g1=0.01, g2=1000):
		...
		if kind == 'log':
			...
			J  = 0.5 * (u*S.dx(0) + v*S.dx(1) - (w + S_ring))**2 * dJ
			Jp = 0.5 * (um*S.dx(0) + vm*S.dx(1) - (wm + S_ring))**2 * dJ
	...
	def solve(self, annotate=False):
		...
		# compute solution :
		#s = solve(self.resid == 0, self.U, J = self.mom_Jac,
		...
		if params['project_boundary']:
			...
			print_min_max(model.u_s, 'U3_S', cls=self)
			...
			print_min_max(model.u_b, 'U3_B', cls=self)
	...
	def update_model_var(self, u, annotate=False):
		...
		to those given by ``u``.
		"""
		self.model.u.assign(u, annotate=annotate)
